...
export async function buildRuntime({ outputDirectory = distDirectory } = {}) {
	...
	try {
		const result = await build({
			...
			entryNames: "pi-starship",
			entryPoints: ["src/pi-starship.ts"],
			format: "esm",
			legalComments: "none",
			metafile: true,
			outExtension: { ".js": ".ts" },
...
export function validateEagerGraph(metafile) {
	...
	const entry = Object.entries(outputs).find(([, output]) =>
		normalizePath(output.entryPoint ?? "").endsWith("/src/pi-starship.ts"),
	);
	if (!entry) throw new Error("Generated runtime metadata has no pi-starship entrypoint");
...
export async function validateGeneratedFiles(outputDirectory) {
	...
	if (!runtimeFiles.some((path) => path === "pi-starship.ts")) {
		throw new Error("Generated runtime is missing pi-starship.ts");
	}
