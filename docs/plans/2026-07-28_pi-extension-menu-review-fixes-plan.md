# Pi Extension Menu Review Fixes
...
- [x] Harden `packages/pi-extension-menu` at shared boundaries: unrestricted Pi peers, direct binding forwarding, raw/display setting separation, committed toggle rollback, unique RPC choices, pending-task draining, and owner `AbortSignal` propagation; remove unsupported settings search until Pi exposes focus forwarding.
...
- [ ] Audit the full PR diff against `docs/extension-conventions.md` and `docs/extension-settings.md`, reply to and resolve all review threads, then commit and push the focused changes.

## Completion Checklist

- [ ] All ten PR review threads are addressed with code, tests, or evidence and resolved on GitHub.
...
- [ ] The review-fix commit is pushed to `feat/pi-extension-menu`, and the worktree is clean.
