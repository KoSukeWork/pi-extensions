...
- Store the allowed catalog per session manager in a package-owned shared weak registry, with a per-`ExtensionAPI` runtime mirror, so real reloads preserve policy without leaking across sessions.
- Rebuild the allowed loaded subset from `firecrawl_load` results on the active branch during session start so resume, fork, and branch switches restore the right capability definitions.
...
- Deterministic deduplicated exact-term ranking, catalog-first filtering, morphology cases, and bidirectional crawl-companion tests cover query selection; regressions exposed both `web` matching `website` and repeated words distorting relevance.
...
- Generation checks now guard session start, command continuations, selector work, and status reads after every relevant await; session-manager ownership preserves unsaved catalogs across real reloads, active-branch results restore allowed loaded tools, and shutdown still waits for settings durability and artifact cleanup.
- User-facing terminology consistently distinguishes available catalog entries from loaded capabilities, stale settings notices are cleared, terminal controls are stripped, and display truncation preserves complete Unicode code points.
...
- [x] Add `packages/pi-firecrawl/src/lazy-tools.ts` with `firecrawl_load`, deterministic search metadata, session-owned availability across API replacement, active-branch loaded-tool reconstruction, bounded constant-size results, cancellation checks, loader-only initialization, additive activation, and helpers for availability changes. Evidence: factory action-method safety, registration, schema, cancellation, no-network, repeat-load, real reload replacement, active-branch-only restore, catalog-first limit, morphology, task distinction, crawl companion, and status-only tests pass.
- [x] Update `packages/pi-firecrawl/src/firecrawl.ts` to register the loader, initialize the unsaved catalog from Pi’s current policy, apply valid saved settings as availability, keep invalid or missing settings side-effect free, restore allowed branch-loaded tools, and defer all other capabilities after each current `session_start`. Evidence: missing, valid, invalid, canonical/legacy precedence, replacement-generation, fork restoration, and shutdown suites pass.
...
- [x] Saved `tools` values control availability, loaded subsets restore from the active branch across session replacement, and disabling availability unloads affected capabilities.
