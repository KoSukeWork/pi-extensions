...
- `packages/pi-plan-mode/src/plan-mode.ts` is already near the 1,000-line boundary, so lifecycle
...
- [x] Implement the settings schema and persistence patches in `packages/pi-plan-mode/src/settings.ts`, preserving side-effect-free reads and existing concurrency guarantees; evidence: 12 focused settings tests passed, including three repeated deterministic concurrency runs.
...
- [x] Implement the approved flat Settings experience in `packages/pi-plan-mode/src/settings-menu.ts`, with immediate atomic feedback and no retroactive change to current implementation state; evidence: 9 TUI/RPC Settings tests passed, including active-state capture and immediate next-export integration.
...
- [x] Update `packages/pi-plan-mode/README.md` with user-facing labels, defaults, application timing, JSON values, resolved-path behavior, retention/compaction trade-offs, explicit-route precedence, failure recovery, and supported modes; evidence: README behavior and JSON values match focused tests and the RPC smoke.
