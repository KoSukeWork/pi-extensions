...
- Keep `SideThread` ephemeral and return a plain handoff result from `runBtwThread`; the `/btw` command handler alone loads the resulting draft into Pi's main editor.
...
- Represent handoff content as ordered user/assistant text segments. Build every quick scope and custom range through the same deterministic formatter.
- Implement custom selection over raw source lines rather than rendered Markdown rows. `Space` anchors a contiguous range, arrows extend it, `Enter` confirms, `Esc` returns, and `Ctrl+C` closes without handoff.

## Non-Goals

- Automatically sending the draft to the main agent.
- Persisting the side thread or handoff selection.
...
- [x] Add focused failing tests for handoff scope extraction, deterministic formatting, custom line-range selection, cancellation, and no default main-thread mutation; red phase: `tsc -p tsconfig.test.json` failed on the intentionally missing handoff module and promote action.
- [x] Add pure handoff segment/range helpers and a width-safe `Space`-anchored selector component; focused compiled pi-btw tests pass, including scrolling and terminal-control escaping.
...
- [x] Update `extensions/pi-btw/README.md` with the opt-in handoff workflow, exact selection controls, draft behavior, and limitations; documented controls match the tested UI.
...
- Closing the custom component before editor loading could lose context on a stale session. Keep the command flow synchronous after handoff selection and avoid session replacement APIs.
