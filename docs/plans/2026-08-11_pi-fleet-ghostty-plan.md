...
- Deterministic result: 17 Pi Fleet test files and 53 tests pass, including separate-process transport and real-child launch integration.
...
- Hardening result: the adversarial pass fixed the macOS Unix-socket path budget, AppleScript launcher command syntax, launcher lifetime, stale async results, concurrent membership start, in-flight shutdown draining, failed-delivery deduplication, concurrent kickoff admission, reload kickoff state, private endpoint modes, carriage-return display controls, partial Ghostty cancellation, and reusable-invite documentation.
