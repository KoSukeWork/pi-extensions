...
> **Certification fork:** `git:` install of this repository loads only `pi-btw`, `pi-goal`, `pi-plan-mode`, and `pi-sync`. Other packages (including `pi-lsp`) remain in the tree for upstream sync but are not in `package.json` `pi.extensions`. See [CERTIFICATION.md](./CERTIFICATION.md).
